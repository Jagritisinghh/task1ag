...
const createFeatures=async (req,res)=>{
    try{
        ...
        const createFeatures=await Plan.create([
            { plan: "Basic", price: 20000, features: basicFeatures },
            { plan: "Premium", price: 40000, features: premiumFeatures },
            { plan: "VIP", price: 70000, features: vipFeatures }
        ]);
        res.status(201).json({
...
const getFeatures = async (req, res) => {
    try {
        const features = await Plan.find({});
        res.status(200).json({
        ...
        res.status(500).json({ message: "An error occurred while fetching features.", error: error.message });
    }
};


module.exports = { createFeatures, getFeatures };